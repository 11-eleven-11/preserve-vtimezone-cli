...
const tzurl = require('../commands/tzurl')
...
module.exports = {
    ...
    const result = await tzurl.buildTzidIndex(`index.html`, `zoneinfo/`, tzidMap, async (ics) => {
      ...
      await new Promise((resolve, reject) => {
        ...
        const eventCallback = tx => {
          if (tx.sha256 === tzidMap.files[file].sha256) {
            this.events.removeListener('tx', eventCallback)
            // Delay 1 second per 50k
            setTimeout(resolve, tzidMap.files[file].size / 500)
          }
        }
  ...
  deployTzidMap(argv, tzidMap) {
    console.log(`Deploying: ${ chalk.green.bold('@tzidmap') }`)
    const payload = [
      bProtocolPrefix, tzidMap.toString(), 'application/json', 'UTF-8', 'vtimezonehashes.json'
    ];
