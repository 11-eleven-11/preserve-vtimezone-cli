...
const tzurl = {
   ...
   * @param {*} resultArray []
   */
  async buildTzidIndex(currentUrl, currentPath, resultArray, unpreservedConsumer) {
    if (!currentUrl) {
      return;
    }
    ...
    if (currentUrl.endsWith('index.html')) {
      ...
      .then(async r => {
        ...
        return Promise.all(lis.toArray().map(l => {
          ...
          if (hrefOfItem.endsWith('index.html')) {
            ...
            return this.buildTzidIndex(hrefOfItem.split('/')[1], currentPath + newPath + '/', resultArray, unpreservedConsumer);
          } else if (hrefOfItem.endsWith('ics')) {
            return this.buildTzidIndex(hrefOfItem, currentPath, resultArray, unpreservedConsumer);
          } else {
      ...
      .then(async r => {
        if (r.preserved.data) {
          ...
          unpreservedConsumer(r.icsPayload);
        }
      });
    } else {
      return;
